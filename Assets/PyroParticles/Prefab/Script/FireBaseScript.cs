    ...
    public class FireBaseScript : MonoBehaviour
    {
       // public static GameObject shot;
        ...
        public static void CreateExplosion(Vector3 pos, float radius, float force)
        {
            ...
            //GameObject gshot = Instantiate(shot, transform.position, transform.rotation) as GameObject;
            RaycastHit hit;
            Collider[] objects = UnityEngine.Physics.OverlapSphere(pos, radius);
            foreach (Collider h in objects)
            {
                //if(Physics.Raycast(h,out hit,100f))
                    if (h.transform.tag=="enemy")
                {
                    h.transform.GetComponent<enemy_health>().removeHealth(50f);
                }
